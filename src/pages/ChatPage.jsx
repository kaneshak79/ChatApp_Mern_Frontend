...
// // // // } else {
// // // //   // For 1-on-1 chat
// // // //   const res = await API.post("/api/chat", { userId: id });
// // // //   data = res.data;
// // // // }
...
// // // //   onClick={() => {
// // // //     if (chat?.isGroupChat) {
// // // //       nav(`/chat/${chat._id}/members`); // Navigate to group members
// // // //     } else {
// // // //       nav(`/user/${otherUser?._id}`); // Navigate to user profile
// // // //     }
// // // //   }}
...
// // // //     src={
// // // //       chat?.isGroupChat
// // // //         ? `https://ui-avatars.com/api/?name=${encodeURIComponent(chat.chatName || "Group")}`
// // // //         : otherUser?.avatar || `https://i.pravatar.cc/150?u=${id}`
// // // //     }
// // // //     className="w-10 h-10 rounded-full"
// // // //   />
// // // //   <h3 className="text-lg font-semibold">
// // // //     {chat?.isGroupChat ? chat.chatName || "Unnamed Group" : otherUser?.name || "User"}
// // // //   </h3>
// // // // </div>
// // // //   ):
...
// // // export default function ChatPage() {
// // //   const { id } = useParams(); // other user's _id or group _id
// // //   const nav = useNavigate();
// // //   const { user: loggedUser } = useContext(AuthContext);
...
// // //       // Try to get chat info from backend
// // //       const res = await API.get(`/api/chat/${id}`);
// // //       data = res.data;
...
// // //       const sorted = msgRes.data.sort(
// // //         (a, b) => new Date(a.createdAt) - new Date(b.createdAt)
// // //       );
// // //       setMessages(sorted);

// // //       // Set other user for 1-on-1 chat
// // //       if (!data.isGroupChat) {
...
// // //   // ✅ Optional: fetch other user info (fallback)
// // //   const fetchUser = async () => {
// // //     if (!id) return;
// // //     try {
// // //       const { data } = await API.get(`/api/user/search?search=`);
...
// // //         {/* LEFT: Chat Name & Avatar */}
// // //         <div
// // //           onClick={() => {
// // //             if (chat?.isGroupChat) {
// // //               nav(`/chat/${chat._id}/members`);
// // //             } else {
// // //               nav(`/user/${otherUser?._id}`);
// // //             }
// // //           }}
// // //           className="flex items-center gap-3 cursor-pointer"
// // //         >
...
// // export default function ChatPage() {
// //   const { id } = useParams(); // userId or groupId
// //   const nav = useNavigate();
// //   const { user: loggedUser } = useContext(AuthContext);
...
// //       // Try fetching group chat first
// //       const groupRes = await API.get(`/api/chat/group/${id}`);
// //       if (groupRes.data) {
// //         data = groupRes.data;
// //       } else {
// //         // Fallback: 1-on-1 chat
// //         const oneOnOneRes = await API.post("/api/chat", { userId: id });
...
// //       // Fetch messages
// //       const msgRes = await API.get(`/api/message/${data._id}`);
// //       setMessages(msgRes.data.sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt)));

// //       // For 1-on-1, set the other user
// //       if (!data.isGroupChat) {
// //         const userInChat = data.users.find(u => u._id !== loggedUser._id);
// //         setOtherUser(userInChat);
// //       }
...
// //       // fallback: list member names excluding logged-in user
// //       const otherNames = chat.users
// //         .filter(u => u._id !== loggedUser._id)
// //         .map(u => u.name)
// //         .join(", ");
...
// //           onClick={() => {
// //             if (chat?.isGroupChat) {
// //               nav(`/chat/group/${chat._id}`); // go to group members page
// //             } else {
// //               nav(`/user/${otherUser?._id}`); // go to user profile
// //             }
// //           }}
// //           className="flex items-center gap-3 cursor-pointer"
// //         >
...
// //         {/* RIGHT: Call buttons for 1-on-1 chats */}
// //         <div className="flex gap-2">
// //           {!chat?.isGroupChat && (
// //             <>
// //               <button
...
//         }
//       } catch {
//         // Ignore group chat errors
//       }

//       // If not group chat, check for existing 1-on-1
//       if (!data) {
//         const oneOnOneRes = await API.get(`/api/chat?userId=${id}`);
//         data = oneOnOneRes.data;

//         // Only create if no chat exists
//         if (!data) {
